...
int SetAmbientCapabilities(const AppSpawningCtx *property, uint64_t extAmbientCaps);
uint64_t GetExtPermCaps(const AppSpawningCtx *property, uint64_t *ambientCaps);
void GetExtPermGids(const AppSpawningCtx *property, uint32_t gidTable[], uint32_t *gidCount, uint32_t maxGids);
