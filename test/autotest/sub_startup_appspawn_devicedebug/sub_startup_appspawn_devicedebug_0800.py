...
SubStartupAppspawnDevicedebug0800(TestCase):
    ...
    def process(self):
        devicedebug_help_dict = [
            "devicedebug: 'aaa' is not a valid devicedebug command. See 'devicedebug help'.",
            "usage: devicedebug <command> <options>",
            "These are common devicedebug commands list:",
            "         help              list available commands",
            "         kill              send a signal(1-64) to a process"
        ]
        result_dict = list(filter(lambda x: x != "\r", self.driver.shell("devicedebug aaa").split("\n")))
