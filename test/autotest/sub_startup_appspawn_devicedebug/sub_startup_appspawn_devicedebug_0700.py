...
SubStartupAppspawnDevicedebug0700(TestCase):
    ...
    def process(self):
        ...
        hap_path = Common.sourcepath('debug_fork.hap', "sub_startup_appspawn_devicedebug")
        ...
        result01 = self.driver.shell("ps -ef|grep exe_sys_cmd|grep -v grep").split("\n")
        childprocess_pid = result01[1].split()[1]
        ...
        self.driver.shell("devicedebug kill -9 %s" % childprocess_pid)

        Step("匹配日志")
        device_logger.stop_log()
        time.sleep(10)
        assert device_logger.check_log("appspawn devicedebug get app info unsuccess, pid=%s" % childprocess_pid)
        assert device_logger.check_log("devicedebug manager process exit. ret=218103837")
