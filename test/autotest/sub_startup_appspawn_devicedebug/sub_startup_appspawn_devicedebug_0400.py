...
SubStartupAppspawnDevicedebug0400(TestCase):
    ...
    def process(self):
        ...
        for i in range(len(signal_lst)):
            assert device_logger.check_log("devicedebug manager process start.")
            assert device_logger.check_log("devicedebug manager process exit. ret=22")
        assert device_logger.check_log("signal is 65 not in [1, 64]")
        assert device_logger.check_log("signal is 0 not in [1, 64]", expect_match_times=len(signal_lst)-1)
