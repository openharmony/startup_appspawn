...
class SubStartupAppspawnNativespawn1800(TestCase):
    ...
    def test_step1(self):
        ...
        time.sleep(1)
        Step("步骤6:再次点击start Native Process Isolated按钮")
        self.driver.touch(BY.text("start Native Process Isolated"))
        Step("步骤7:第二次获取nativesapwn进程号")
        pid2 = self.driver.System.get_pid("nativespawn")
        Step("步骤8:杀掉孵化的一个nativ进程")
        child_pid = self.driver.System.get_pid("com.acts.childprocessmanager:Native_libentry0")
        self.driver.shell("kill -9 %d" % child_pid)
        Step("步骤9:第三次获取nativesapwn进程号")
        pid3 = self.driver.System.get_pid("nativespawn")
        Step("步骤10:杀掉孵化的最后一个native子进程")
        child_pid 1 = self.driver.System.get_pid("com.acts.childprocessmanager:Native_libentry1")
        self.driver.shell("kill -9 %d" % child_pid 1)
        Step("步骤11:第四次获取nativesapwn进程号")
