...
class SubStartupAppspawnNativespawn0900(TestCase):
    ...
    def test_step1(self):
        ...
        self.driver.shell("echo 0123456789 > /proc/%d/root/data/storage/el1/123.txt" % pid)
        Step("步骤8:测试应用孵化native进程查看沙箱路径data/storage/el1")
        childres = self.driver.shell("cat /proc/%d/root/data/storage/el1/123.txt" % child_pid)
        Step("步骤8:预期结果检验")
        self.driver.Assert.contains(childres, "root/data/storage/el1/123.txt: No such file or directory")
