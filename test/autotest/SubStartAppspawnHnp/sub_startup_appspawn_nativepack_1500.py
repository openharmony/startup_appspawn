...
class SUB_STARTUP_APPSPAWN_NATIVEPACK_1500(TestCase):
    ...
    # 打包native包时，传参异常
    def test_step1(self):
        Step("调用./hnpcli 待打包native软件路径 打包后文件存放路径 -n test")
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {self.pack_out_path} -n test")
        ...
        Step("调用./hnpcli 待打包native软件路径 打包后文件存放路径 -v 1.1")
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {self.pack_out_path} -v 1.1")
        self.driver.Assert.contains(result, "[ERROR][HNP]name or version argv is miss.")
        self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=8392706")
        Step("调用./hnpcli pack native包路径 存储包目标路径 -n "" -v """)
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {self.pack_out_path} -name ""  -v """)
        self.driver.Assert.contains(result, "[ERROR][HNP]name or version argv is miss.")
        self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=8392706")
