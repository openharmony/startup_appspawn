...
class SUB_STARTUP_APPSPAWN_NATIVEINSTALL_2800(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            ...
            Step("校验安装工具可用")
            result = self.driver.shell(
                f"./{self.default_install_root_path}/300/hnp/baidu/bin/printfHello 2")
            self.driver.Assert.contains(result, "")

            result = self.driver.shell(
                f"./{self.default_install_root_path}/300/hnp/meituan/bin/printfHello 2")
            self.driver.Assert.contains(result, "")
            ...
            result = self.driver.shell(
                ...
                f"-o {self.pack_out_path}/private -n printfSymbol -v 1.0")
            self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=0")

            Step("强制安装elema应用软件安装程序")
            result = self.driver.shell(
                f"{self.hnp_commamd} install -u 300 -p elema -i {self.pack_out_path} -f")
            self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=0")
            ...
            result = self.driver.shell(
                f"./{self.default_install_root_path}/300/hnp/elema/bin/printfHello 2")
            self.driver.Assert.contains(result, "****************hello**************")
            ...
            result = self.driver.shell(
                f"./{self.default_install_root_path}/300/hnp/elema/bin/printfSymbol")
            self.driver.Assert.contains(result, "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
            ...
            Step("卸载饿了吗软件")
            result = self.driver.shell(
                f"{self.hnp_commamd} uninstall -u 300 -p elema")
            self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=0")
            ...
            result = self.driver.shell(
                f"./{self.default_install_root_path}/300/hnp/elema/bin/printfSymbol")
            self.driver.Assert.contains(result, "printfSymbol: inaccessible or not found")
