...
class SUB_STARTUP_APPSPAWN_NATIVEINSTALL_1800(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            Step("创建一个临时native包")
            os.makedirs(f"{self.current_path}/test/")
            os.makedirs(f"{self.current_path}/test/bin")
            Common.writeDateToFile(f"{self.current_path}/test/bin/test.txt", "testtesttesttest")

            Step("使用zip工具压缩native包，名称为test.hnp")
            Common.zip_folder("test", f"{self.current_path}/test.hnp")
            self.driver.push_file(f"{self.current_path}\\test.hnp", f"{self.pack_out_path}/public")
            Common.deleteFile(f"{self.current_path}\\test.hnp")
            shutil.rmtree(f"{self.current_path}/test/")

            Step("安装test.hnp包")
            result = self.driver.shell(
                f"{self.hnp_commamd} install -u 300 -p baidu -i {self.pack_out_path} -f")
            self.driver.Assert.contains(result, "[ERROR][HNP]hnp cfg file not found.")
        else:
            pass
