...
class SUB_STARTUP_APPSPAWN_NATIVEPACK_1900(TestCase):
    ...
    def test_step1(self):
        ...
        hnp_cfg_data['install']['links'].append({"source": "bin/sample.txt", "target": "hnp_test"})
        ...
        self.driver.Assert.contains(result, "linkNum=2, ret=0")
        self.check_pack_is_correct("hnp_test.hnp", "sample1", f"{self.current_path}/hnp.json", False)
        Common.deleteFile(f"{self.current_path}/hnp.json")
        self.driver.shell(f"rm {self.native_path[0]}/hnp.json")
        Step("配置文件字段正确，存在两个软连接，相同的软连接路径，不同的配置名称")
        hnp_cfg_data = Common.readJsonFile("testFile\SUB_STARTUP_APPSPAWN_HNP\hnp.json")
        hnp_cfg_data['install']['links'].append({"source": "bin/sample.txt", "target": "hnp_test2"})
        self.creat_json_and_push_remote("hnp.json", hnp_cfg_data, self.native_path[0], False)
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {self.pack_out_path}")
        self.driver.Assert.contains(result, "linkNum=2, ret=0")
        ...
        Common.deleteFile(f"{self.current_path}/hnp.json")
        self.driver.shell(f"rm {self.native_path[0]}/hnp.json")
        Step("配置文件字段正确，存在多个软连接，相同的软连接路径，相同的配置名称")
        hnp_cfg_data = Common.readJsonFile("testFile\SUB_STARTUP_APPSPAWN_HNP\hnp.json")
        for i in range(5):
            hnp_cfg_data['install']['links'].append({"source": "bin/sample.txt", "target": "hnp_test"})
        self.creat_json_and_push_remote("hnp.json", hnp_cfg_data, self.native_path[0], False)
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {self.pack_out_path}")
        self.driver.Assert.contains(result, "linkNum=6, ret=0")
        self.check_pack_is_correct("hnp_test.hnp", "sample1", f"{self.current_path}/hnp.json", False)
        Common.deleteFile(f"{self.current_path}/hnp.json")
        self.driver.shell(f"rm {self.native_path[0]}/hnp.json")
    def teardown(self):
