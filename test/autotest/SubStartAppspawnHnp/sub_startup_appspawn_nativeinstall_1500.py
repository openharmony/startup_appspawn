...
class SUB_STARTUP_APPSPAWN_NATIVEINSTALL_1500(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            Step("创建配置文件，hnp中存在3个bin文件，2个软链接，软链接路径分别指向其中bin文件")
