...
class SUB_STARTUP_APPSPAWN_NATIVEINSTALL_0600(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            Step("打包两个工具 compute & printfHello")
            ...
            result = self.driver.shell(
                f"{self.hnpcli_commamd} pack -i {self.native_path[1]} -o {self.pack_out_path}/public -n printfHello -v 1.0")
            self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=0")

            Step("批量安装compute、printfHello工具")
            result = self.driver.shell(
            ...
            self.driver.Assert.contains(result, "hnp uninstall end! ret=0")
