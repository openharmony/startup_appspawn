...
class SUB_STARTUP_APPSPAWN_NATIVEINSTALL_1300(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            Step("创建配置文件，存在一个软链接，软链接为文件夹")
            hnp_cfg_data = Common.readJsonFile("testFile\SUB_STARTUP_APPSPAWN_HNP\hnp.json")
            hnp_cfg_data['install']['links'][0]["source"] = "bin"
            hnp_cfg_data['install']['links'][0]["target"] = "printfSymbol_bin"
            hnp_cfg_data['name'] = "printfSymbol_bin"
            hnp_cfg_data['version'] = "1.0"
            self.creat_json_and_push_remote("hnp.json", hnp_cfg_data, self.native_path)

            Step("打包printfSymbol_bin.hnp")
            ...
            Step("强制安装printfSymbol_bin.hnp")
            result = self.driver.shell(
                f"{self.hnp_commamd} install -u 300 -p baidu -i {self.pack_out_path} -f")
            self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=0")

            Step("校验安装printfSymbol_bin.hnp是否成功，是否存在printfSymbol_bin软链")
            result = self.driver.shell(f"find {self.default_install_root_path}/300/{self.hnp_public_dir} "
                                       f"-name printfSymbol_bin")
            self.driver.Assert.contains(result, "printfSymbol_bin")
