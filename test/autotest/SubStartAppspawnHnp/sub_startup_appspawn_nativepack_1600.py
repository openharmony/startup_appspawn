...
class SUB_STARTUP_APPSPAWN_NATIVEPACK_1600(TestCase):
    ...
    def test_step1(self):
        ...
        self.check_pack_is_correct("hnp_test.hnp", "sample1", f"{self.hnp_test_file_path}\\hnp.json", False)
        self.driver.shell(f'rm {self.native_path[0]}/hnp.json')
        Step("打包文件不存在cfg文件，传入name与version")
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {self.pack_out_path} -n test -v 1.0")
        self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=0")
        ...
        Step("打包文件不存在cfg文件，传入name与version，其中name与第一次传入不同")
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {self.pack_out_path} -n test2 -v 1.0")
        self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=0")
        result = self.driver.shell(f"find {self.pack_out_path} -name test2.hnp")
        self.driver.Assert.contains(result, f"{self.pack_out_path}/test2.hnp")
        Step("调用./hnpcli 待打包native软件路径（与上不同）打包后文件存放路径（与上相同）-n test -v 1.0")
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[1]} -o {self.pack_out_path} -n test -v 1.0")
        self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=0")
        result = self.driver.shell(f"find {self.pack_out_path} -name test.hnp")
        self.driver.Assert.contains(result, f"{self.pack_out_path}/test.hnp")
