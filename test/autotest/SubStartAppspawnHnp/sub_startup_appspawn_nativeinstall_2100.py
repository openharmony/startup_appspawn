...
class SUB_STARTUP_APPSPAWN_NATIVEINSTALL_2100(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            ...
            Step("创建readCfgFile工具配置文件，软连接名称为readCfgFile")
            hnp_cfg_data = Common.readJsonFile("testFile\SUB_STARTUP_APPSPAWN_HNP\hnp.json")
            hnp_cfg_data['install']['links'][0]["source"] = "bin/readCfgFile"
            hnp_cfg_data['install']['links'][0]["target"] = "readCfgFile"
            hnp_cfg_data['name'] = "readCfgFile"
            hnp_cfg_data['version'] = "1.0"
            ...
            self.creat_json_and_push_remote("hnp.json", hnp_cfg_data, self.native_path[2])

            result = self.driver.shell(
                f"{self.hnpcli_commamd} pack -i {self.native_path[2]} -o {self.pack_out_path}/public")
            self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=0")

            Step("重复非强制安装readCfgFile 2.0版本工具")
            result = self.driver.shell(
                f"{self.hnp_commamd} install -u 300 -p baidu -i {self.pack_out_path}")
            self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=8393476 ")

            Step("重复强制安装readCfgFile 2.0版本工具")
            result = self.driver.shell(
            ...
            result = self.driver.shell(
                f"./{self.default_install_root_path}/300/{self.hnp_public_dir}/printfHello 2")
            self.driver.Assert.contains(result, "****************hello**************")

            result = self.driver.shell(
                f"./{self.default_install_root_path}/300/{self.hnp_public_dir}/readCfgFile")
            self.driver.Assert.contains(result, "readCfgFile: inaccessible or not found")

            result = self.driver.shell(
                f"find {self.default_install_root_path}/300/hnppublic/readCfgFile.org/ -name readCfgFile_1.0")
            assert ("multiplication" not in result) == True

            result = self.driver.shell(
                f"find {self.default_install_root_path}/300/hnppublic/readCfgFile.org/ -name readCfgFile_2.0")
            self.driver.Assert.contains(result, "readCfgFile_2.0")
