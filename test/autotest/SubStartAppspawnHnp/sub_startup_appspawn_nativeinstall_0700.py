...
class SUB_STARTUP_APPSPAWN_NATIVEINSTALL_0700(TestCase):
    ...
    # 打包文件未设置软链接路径，安装hnp包时可执行文件在非bin目录下场景
    def test_step1(self):
        Step("识别设备型号...............................")
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            Step("创建一个空目录temp_dir,该目录下存在一个二进制文件printfHello")
            self.driver.shell(f"mkdir {self.work_space_path}/temp_dir/")
            self.driver.shell(f"cp {self.native_path}/bin/*  {self.work_space_path}/temp_dir/")

            Step("打包temp_dir为test.hnp包")
            result = self.driver.shell(f"{self.hnpcli_commamd} pack -i {self.work_space_path}/temp_dir/ "
