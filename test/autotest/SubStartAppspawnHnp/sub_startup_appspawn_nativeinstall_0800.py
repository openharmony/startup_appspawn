...
class SUB_STARTUP_APPSPAWN_NATIVEINSTALL_0800(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            Step("创建一个temp_dir目录，创建bin/bin目录，bin目录下分别存在一个二进制")
            self.driver.shell(f"mkdir {self.work_space_path}/temp_dir/")
            self.driver.shell(f"mkdir {self.work_space_path}/temp_dir/bin/")
            self.driver.shell(f"mkdir {self.work_space_path}/temp_dir/bin/bin/")
            self.driver.shell(f"cp {self.native_path[1]}/bin/printfHello  {self.work_space_path}/temp_dir/bin")
            self.driver.shell(f"cp {self.native_path[0]}/bin/add  {self.work_space_path}/temp_dir/bin/bin")

            Step("打包该native文件为test.hnp")
            result = self.driver.shell(
                f"{self.hnpcli_commamd} pack -i {self.work_space_path}/temp_dir/ -o "
                f"{self.pack_out_path}/public -n test -v 1.0")
            ...
            Step("校验bin/printfHello工具生效，bin/bin/add未生效")
            result = self.driver.shell(
