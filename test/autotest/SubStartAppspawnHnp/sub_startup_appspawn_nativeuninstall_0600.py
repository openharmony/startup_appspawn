...
class SUB_STARTUP_APPSPAWN_NATIVEUNINSTALL_0600(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            ...
            hnp_cfg_data['install']['links'].append({"source": "bin/del", "target": "del"})
            hnp_cfg_data['install']['links'].append({"source": "bin/multiplication", "target": "multiplication"})
            ...
            result = self.driver.shell(
                f"./{self.default_install_root_path}/300/hnp/baidu/bin/multiplication")
            self.driver.Assert.contains(result, "multiplication: inaccessible or not found")

            Step("验证printfHello工具卸载成功")
            result = self.driver.shell(
