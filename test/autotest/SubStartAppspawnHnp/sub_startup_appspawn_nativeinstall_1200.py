...
class SUB_STARTUP_APPSPAWN_NATIVEINSTALL_1200(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            Step("创建配置文件，存在一个软链接，软链接类型.txt文件")
            hnp_cfg_data = Common.readJsonFile("testFile\SUB_STARTUP_APPSPAWN_HNP\hnp.json")
            hnp_cfg_data['install']['links'][0]["source"] = "bin/sample.txt"
            hnp_cfg_data['install']['links'][0]["target"] = "sample"
            hnp_cfg_data['name'] = "sample"
            ...
            Step("验证hnp/hnppublic/bin目录下是否存在sample")
            result = self.driver.shell(f"find {self.default_install_root_path}/300/{self.hnp_public_dir} -name sample")
            self.driver.Assert.contains(result, f"sample")
