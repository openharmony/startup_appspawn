...
class SUB_STARTUP_APPSPAWN_NATIVEINSTALL_2700(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            ...
            result = self.driver.shell(
                f"{self.hnpcli_commamd} pack -i {self.native_path[3]} "
            ...
            result = self.driver.shell(
                ...
                f"-o {self.pack_out_path}/public -n printfSymbol -v 1.0")
            self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=0")
            ...
            Step("非强制重复安装软件")
            self.driver.shell(f'rm {self.pack_out_path}/public/compute.hnp')
            result = self.driver.shell(
                f"{self.hnp_commamd} install -u 300 -p baidu -i {self.pack_out_path}")
    ...
    def teardown(self):
        ...
        self.driver.shell(f'rm /data/service/el1/startup/hnp_info.json')
        Step("收尾工作")
