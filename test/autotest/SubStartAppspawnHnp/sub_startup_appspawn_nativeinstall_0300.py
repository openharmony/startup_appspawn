...
class SUB_STARTUP_APPSPAWN_NATIVEINSTALL_0300(TestCase):
    def __init__(self, controllers):
        ...
        self.hnp_private_dir = "hnp/baidu/bin"
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            Step("安装软件包时，hnp包路径不存在")
            result = self.driver.shell(
                f"{self.hnp_commamd} install -u 300 -p baidu -i /data/empty_path/ -f")
            self.driver.Assert.contains(result, "[ERROR][HNP]hnp install opendir:/data/empty_path/ unsuccess, errno=2")
            self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=8392968")

            Step("安装软件包时，hnp包路径长度等于4096")
            dir_temp = "/data/" + Common.randomGenerateStingByLen(4090)
            result = self.driver.shell(
                f"{self.hnp_commamd} install -u 300 -p baidu -i {dir_temp} -f")
            self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=8392968")

            Step("安装软件包时，hnp包路径长度大于4096")
            dir_temp = "/data/" + Common.randomGenerateStingByLen(4096)
            result = self.driver.shell(
                f"{self.hnp_commamd} install -u 300 -p baidu -i {dir_temp} -f")
            self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=8392968")

            Step("安装软件包时，hnp包路径中存在非法字符")
            result = self.driver.shell(
                f"{self.hnp_commamd} install -u 300 -p baidu -i /data/empty**#…………_path/ -f")
            self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=8392968")

            Step("安装软件包时，安装路径为空文件夹")
            result = self.driver.shell(
            ...
            Step("安装软件包时没有公有或者私有数据文件夹")
            result = self.driver.shell(
                f"{self.hnpcli_commamd} pack -i {self.native_path} -o {self.pack_out_path} -n compute -v 1.3")
            self.driver.Assert.contains(result, "linkNum=0, ret=0")
            result = self.driver.shell(f"find {self.pack_out_path} -name compute.hnp")
            self.driver.Assert.contains(result, f"{self.pack_out_path}/compute.hnp")
            ...
            Step("验证软链接是否生效")
            result = self.driver.shell(
                f"./{self.default_install_root_path}/300/{self.hnp_public_dir}/add 2")
            self.driver.Assert.contains(result, "add: inaccessible or not found")

            result = self.driver.shell(
                f"./{self.default_install_root_path}/300/{self.hnp_private_dir}/add 2")
            self.driver.Assert.contains(result, "add: inaccessible or not found")
