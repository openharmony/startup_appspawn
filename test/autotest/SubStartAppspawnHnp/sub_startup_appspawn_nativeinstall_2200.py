...
class SUB_STARTUP_APPSPAWN_NATIVEINSTALL_2200(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            Step("创建compute工具配置文件，工具add的软连接名称为compute，工具del的软连接名称为compute")
            ...
            Step("验证工具是否生效")
            result = self.driver.shell(
                f"./{self.default_install_root_path}/300/{self.hnp_public_dir}/compute")
