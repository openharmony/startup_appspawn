...
class SUB_STARTUP_APPSPAWN_NATIVEINSTALL_2000(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            ...
            result = self.driver.shell(
                f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {self.pack_out_path}/private")
            self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=0")

            Step("打包printfSymbol工具")
            result = self.driver.shell(
                f"{self.hnpcli_commamd} pack -i {self.native_path[1]} -o {self.pack_out_path}/private")
            self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=0")
            ...
            result = self.driver.shell(
                f"./{self.default_install_root_path}/300/hnp/baidu/bin/add")
