...
class SUB_STARTUP_APPSPAWN_NATIVEPACK_1700(TestCase):
    ...
    def test_step1(self):
        ...
        hnp_cfg_data = """{
            ...
            "install": {
                "links": [
                    {
                        "source": "bin/python2.7",
                        "target": "python"
                    }
                ]
            }"""
        Common.writeDateToFile("hnp.json", hnp_cfg_data)
        self.driver.push_file(f"{self.current_path}\\hnp.json", self.native_path[0])
        Common.deleteFile(f"{self.current_path}\\hnp.json")
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {self.pack_out_path}")
        self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=8392986")
        ...
        del hnp_cfg_data['type']
        self.creat_json_and_push_remote("hnp.json", hnp_cfg_data, self.native_path[0])
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {self.pack_out_path}")
        self.driver.Assert.contains(result, "[ERROR][HNP]get type info in cfg unsuccess.")
        self.driver.shell(f"rm {self.native_path[0]}/hnp.json")
        Step("配置文件无name字段")
        hnp_cfg_data = Common.readJsonFile("testFile\SUB_STARTUP_APPSPAWN_HNP\hnp.json")
        del hnp_cfg_data['name']
        self.creat_json_and_push_remote("hnp.json", hnp_cfg_data, self.native_path[0])
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {self.pack_out_path}")
        self.driver.Assert.contains(result, "[ERROR][HNP]get name info in cfg unsuccess.")
        self.driver.shell(f"rm {self.native_path[0]}/hnp.json")
        Step("配置文件无version字段")
        hnp_cfg_data = Common.readJsonFile("testFile\SUB_STARTUP_APPSPAWN_HNP\hnp.json")
        del hnp_cfg_data['version']
        self.creat_json_and_push_remote("hnp.json", hnp_cfg_data, self.native_path[0])
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {self.pack_out_path}")
        self.driver.Assert.contains(result, "[ERROR][HNP]get version info in cfg unsuccess.")
        self.driver.shell(f"rm {self.native_path[0]}/hnp.json")
        Step("配置文件无install字段")
        hnp_cfg_data = Common.readJsonFile("testFile\SUB_STARTUP_APPSPAWN_HNP\hnp.json")
        del hnp_cfg_data['install']
        self.creat_json_and_push_remote("hnp.json", hnp_cfg_data, self.native_path[0])
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {self.pack_out_path}")
        self.driver.Assert.contains(result, "[ERROR][HNP]get install info in cfg unsuccess.")
        self.driver.shell(f"rm {self.native_path[0]}/hnp.json")
        Step("配置文件非hnp类型")
        hnp_cfg_data = Common.readJsonFile("testFile\SUB_STARTUP_APPSPAWN_HNP\hnp.json")
        hnp_cfg_data["type"] = "test"
        self.creat_json_and_push_remote("hnp.json", hnp_cfg_data, self.native_path[0])
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {self.pack_out_path}")
        self.driver.Assert.contains(result, "[ERROR][HNP]type info not match.type=test")
        self.driver.shell(f"rm {self.native_path[0]}/hnp.json")
