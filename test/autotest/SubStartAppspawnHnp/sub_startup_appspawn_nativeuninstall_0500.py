...
class SUB_STARTUP_APPSPAWN_NATIVEUNINSTALL_0500(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            Step("创建compute工具配置文件，工具add的软连接名称为compute，工具add的软连接名称为compute")
            ...
            hnp_cfg_data['install']['links'].append({"source": "bin/del", "target": "compute"})
            hnp_cfg_data['install']['links'].append({"source": "bin/multiplication", "target": "mul"})
