...
class SUB_STARTUP_APPSPAWN_NATIVEINSTALL_2300(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            Step("创建compute工具配置文件，工具del的软连接名称为del_temp，工具add、multiplication无软链接name")
            hnp_cfg_data = Common.readJsonFile("testFile\SUB_STARTUP_APPSPAWN_HNP\hnp.json")
            hnp_cfg_data['install']['links'][0]["source"] = "bin/del"
            hnp_cfg_data['install']['links'][0]["target"] = "del_temp"
            hnp_cfg_data['install']['links'].append({"source": "bin/add"})
            hnp_cfg_data['install']['links'].append({"source": "bin/multiplication"})
            hnp_cfg_data['name'] = "compute_no_target"
            hnp_cfg_data['version'] = "1.0"
            self.creat_json_and_push_remote("hnp.json", hnp_cfg_data, self.native_path)

            Step("打包compute_no_target.hnp")
            ...
            Step("验证工具是否生效")
            result = self.driver.shell(
                f"./{self.default_install_root_path}/300/{self.hnp_public_dir}/del_temp")
