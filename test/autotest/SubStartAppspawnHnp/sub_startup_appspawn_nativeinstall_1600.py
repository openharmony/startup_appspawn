...
class SUB_STARTUP_APPSPAWN_NATIVEINSTALL_1600(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            Step("创建compute工具配置文件，存在3个bin文件，3个软链接，软链接路径分别指向其中bin文件")
            ...
            Step("创建readCfgFile工具配置文件，存在1个bin文件，1个软链接，软链接路径指向bin文件")
            hnp_cfg_data = Common.readJsonFile("testFile\SUB_STARTUP_APPSPAWN_HNP\hnp.json")
            ...
            Step("创建printfSymbol工具配置文件，存在1个bin文件，1个软链接，软链接路径指向bin文件")
            hnp_cfg_data = Common.readJsonFile("testFile\SUB_STARTUP_APPSPAWN_HNP\hnp.json")
            hnp_cfg_data['install']['links'][0]["source"] = "bin/printfSymbol"
            hnp_cfg_data['install']['links'][0]["target"] = "printfSymbol"
