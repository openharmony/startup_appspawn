...
class SUB_STARTUP_APPSPAWN_NATIVEUNINSTALL_0300(TestCase):
    ...
    def test_step1(self):
        ...
        if ("HYM" in device or "HAD" in device or "ohos" in device or "ALN" in device):
            ...
            Step("卸载软件usrId不存在")
            result = self.driver.shell(
                f"{self.hnp_commamd} uninstall -u 301 -p baidu")
            self.driver.Assert.contains(result, "[INFO][HNP]native manager process exit. ret=8393477 ")
