...
class SUB_STARTUP_APPSPAWN_NATIVEPACK_1300(TestCase):
    ...
    # hnp程序打包native包，存储包目标路径相关测试场景
    def test_step1(self):
        Step("存储包目标路径不存在")
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {self.pack_out_path}/empty_dir -n test -v 1.1")
        self.driver.Assert.contains(result, "[ERROR][HNP]realpath unsuccess. path=/data/hnp_test/pack_output/empty_dir")
        Step("存储包目标路径长度等于max_path")
        path_temp = Common.randomGenerateStingByLen(4088)
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {path_temp} -n test -v 1.1")
        self.driver.Assert.contains(result, "[ERROR][HNP]realpath unsuccess.")
        Step("存储包目标路径长度大于max_path")
        path_temp = Common.randomGenerateStingByLen(4097)
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o {path_temp} -n test -v 1.1")
        self.driver.Assert.contains(result, "[ERROR][HNP]realpath unsuccess.")
        Step("/hnpcli pack native包路径 存储包目标路径 -cfg 配置路径，native包路径存在非法字符")
        result = self.driver.shell(
            f"{self.hnpcli_commamd} pack -i {self.native_path[0]} -o /data/$^*%test/ -n test -v 1.1")
        self.driver.Assert.contains(result, "[ERROR][HNP]realpath unsuccess. path=/data/$^*%test/")
