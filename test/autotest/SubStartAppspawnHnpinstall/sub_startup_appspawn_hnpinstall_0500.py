...
class SUB_STARTUP_APPSPAWN_HNPINSTALL_0500(TestCase):
    ...
    def test_step1(self):
        ...
        if ("Permission denied" in isroot):
            ...
            result1 = self.driver.shell(
                f"rm -rf /data/app/el1/bundle/100/hnppublic/hnpsample.org/hnpsample_1.1/hnp.json")
            self.driver.Assert.contains(result1, "Permission denied")
            Step("步骤3：修改私有目录文件内容")
            result2 = self.driver.shell(
                f"rm -rf /data/app/el1/bundle/100/hnp/com.example.hnp_test/node.org/node_1.0/hnp.json")
            self.driver.Assert.contains(result2, "Permission denied")
        else:
            Step("步骤2：修改公有目录文件内容")
            self.driver.shell(f"rm -rf /data/app/el1/bundle/100/hnppublic/hnpsample.org/hnpsample_1.1/hnp.json")
            result1 = self.driver.shell("ls /data/app/el1/bundle/100/hnppublic/hnpsample.org/hnpsample_1.1/")
            self.driver.Assert.is_true("hnp.json" not in result1)
            Step("步骤3：修改私有目录文件内容")
            self.driver.shell(f"rm -rf /data/app/el1/bundle/100/hnp/com.example.hnp_test/node.org/node_1.0/hnp.json")
            result2 = self.driver.shell(
                "ls /data/app/el1/bundle/100/hnp/com.example.hnp_test/node.org/node_1.0/")
            self.driver.Assert.is_true("hnp.json" not in result2)
