...
class SubStartupAppspawnNativespawn0400(TestCase):
    ...
    def test_step1(self):
        ...
        Step("步骤7:查看数据")
        res = self.driver.shell("ls -l /proc/%d/ns/net" % pid)
        childres = self.driver.shell("ls -l /proc/%d/ns/net" % child_pid)
        Step("步骤7:预期结果检验")
        arr = res.split("-> ")[1]
        childarr = childres.split("-> ")[1]
        self.driver.Assert.equal(arr, childarr)
