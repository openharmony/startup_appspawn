...
class SubStartupAppspawnNativespawn1500(TestCase):
    ...
    def test_step1(self):
        ...
        Step("步骤7:杀死nativespawn进程")
        self.driver.shell("kill -9 `pidof nativespawn`")
        Step("步骤8:查询测试应用")
        result = self.driver.shell("ps -ef | grep com.acts.childprocessmanager")
        Step("步骤9:查询nativespawn进程号")
        nativepid = self.driver.System.get_pid("nativespawn")
        self.driver.Assert.contains(result, str(child_pid 1))
        self.driver.Assert.contains(result, str(pid))
        self.driver.Assert.equal(nativepid, None)
