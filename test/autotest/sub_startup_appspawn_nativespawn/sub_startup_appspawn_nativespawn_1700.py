...
class SubStartupAppspawnNativespawn1700(TestCase):
    ...
    def test_step1(self):
        ...
        Step("步骤6:获取测试应用nativespawn孵化子进程pid")
        child_pid = self.driver.System.get_pid("com.acts.childprocessmanager:Native_libentry0")
        Step("步骤7:测试应用selinux标签类型")
        res = self.driver.shell("ps -efZ | grep %d | grep -v grep" % pid)
        Step("步骤8:孵化的native子进程(Isolated=false)selinux标签类型")
        childres = self.driver.shell("ps -efZ | grep %d | grep -v grep" % child_pid)
        string = res.split(" ")[0]
        child_string = childres.split(" ")[0]
        Step("步骤9: 预期结果校验")
        self.driver.Assert.equal(string, child_string)
