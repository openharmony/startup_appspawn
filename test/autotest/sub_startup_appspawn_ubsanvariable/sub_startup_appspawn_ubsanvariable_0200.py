...
class Substartupappspawnubsanvariable0200(TestCase):
    ...
    def test_step1(self):
        ...
        hap = os.path.abspath(os.path.join(os.path.join(path, "testFile"),
                                           "SUB_STARTUP_APPSPAWN_UBSANVARIABLE/ActsStartEnabledFalseProcessTest.hap"))
        ishap = self.driver.has_app("com.example.actsstartenabledtrueprocesstest")
        if (ishap):
            self.driver.uninstall_app("com.example.actsstartenabledfalseprocesstest")
        else:
            pass
        ...
        Step("步骤4：启动测试应用")
        self.driver.shell("aa start -a EntryAbility -b com.example.actsstartenabledfalseprocesstest")
        time.sleep(8)
        Step("步骤4：关闭日志")
