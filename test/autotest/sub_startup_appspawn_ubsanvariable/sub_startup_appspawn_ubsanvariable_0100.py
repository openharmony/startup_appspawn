...
class Substartupappspawnubsanvariable0100(TestCase):
    ...
    def setup(self):
        ...
        Step('开启DEBUG日志')
        self.driver.shell("hilog -b D -D 0xC02C11")
    ...
    def test_step1(self):
        ...
        hap = os.path.abspath(os.path.join(os.path.join(path, "testFile"),
                                           "SUB_STARTUP_APPSPAWN_UBSANVARIABLE/ActsStartEnabledTrueProcessTest.hap"))
        ishap = self.driver.has_app("com.example.actsstartenabledtrueprocesstest")
        if(ishap):
            self.driver.uninstall_app("com.example.actsstartenabledtrueprocesstest")
        else:
            pass
        ...
        Step("步骤4：启动测试应用")
        self.driver.shell("aa start -a EntryAbility -b com.example.actsstartenabledtrueprocesstest")
        time.sleep(8)
        Step("步骤4：关闭日志")
        device_logger.stop_log()
        device_logger.check_log('SetAsanEnabledEnv 22,(null),(null),(null)')
        device_logger.check_log('print_stacktrace=1:print_module_map=2:log_exe_name=1,(null)')
