...
int SetXpmConfig(const AppSpawnMgr *mgr, const AppSpawningCtx *property);
...
namespace OHOS {
static CodeSignTestHelper g_testHelper;
...
HWTEST_F(AppSpawnCodeSignTest, App_Spawn_SetXpmConfig_001, TestSize.Level0)
{
    ...
    do {
        ...
        reqHandle = g_testHelper.CodeSignTestCreateSendMsg(clientHandle, MSG_APP_SPAWN);
        APPSPAWN_CHECK(reqHandle != INVALID_REQ_HANDLE, break, "Failed to create req %{public}s", APPSPAWN_SERVER_NAME);

        property = g_testHelper.CodeSignTestGetAppProperty(clientHandle, reqHandle);
