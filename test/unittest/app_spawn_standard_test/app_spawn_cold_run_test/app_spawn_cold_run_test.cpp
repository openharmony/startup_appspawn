...
StubNode g_stubNodes[] = {
    {STUB_MOUNT, 0, 0, nullptr},
    {STUB_EXECV, 0, 0, nullptr},
};

StubNode *GetStubNode(int type)
{
    if (type >= static_cast<int>(sizeof(g_stubNodes) / sizeof(g_stubNodes[0]))) {
        return nullptr;
    }
...
public:
    ...
    void SetUp()
    {
        ...
        testServer = std::make_unique<OHOS::ServerTestHelper>("appspawn -mode appspawn");
...
static int HandleExecvStub(const char *pathName, char *const argv[])
{
    ...
    AppSpawnContent *content = ServerTestHelper::TestStartSpawnServer(cmd, args);
    if (content == nullptr) {
        free(args);
        return -1;
    }
