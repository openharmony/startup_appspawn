...
namespace OHOS {
void IsolateTestHelper::IsolateTestSetDefaultData(void)
{
    isolateTestProcessName_ = std::string("com.example.isolate.app");
    isolateTestDefaultTestUid_ = DEFAULT_TEST_ID;
    isolateTestDefaultTestGid_ = DEFAULT_TEST_ID;
    isolateTestDefaultTestGidGroup_ = DEFAULT_TEST_ID;
    isolateTestDefaultTestBundleIndex_ = 1;
    isolateTestDefaultApl_ = std::string("system_core");
    isolateTestDefaultMsgFlags_ = 0;
}

int IsolateTestHelper::IsolateTestAddDacInfo(AppSpawnReqMsgHandle &reqHandle)
{
    AppDacInfo dacInfo = {};
    dacInfo.uid = isolateTestDefaultTestUid_;
    dacInfo.gid = isolateTestDefaultTestGid_;
    dacInfo.gidCount = DAC_COUNT_TEST_ID;
    dacInfo.gidTable[0] = isolateTestDefaultTestGidGroup_;
    dacInfo.gidTable[1] = isolateTestDefaultTestGidGroup_ + 1;
    APPSPAWN_CHECK_ONLY_EXPER(strcpy_s(dacInfo.userName, sizeof(dacInfo.userName), "test-isolate-app") == 0,
        return APPSPAWN_ARG_INVALID);
    return AppSpawnReqMsgSetAppDacInfo(reqHandle, &dacInfo);
}

int IsolateTestHelper::IsolateTestAddFdInfo(AppSpawnReqMsgHandle &reqHandle)
{
...
int IsolateTestHelper::IsolateTestSetMsgFlags(AppSpawnReqMsgHandle reqHandle, uint32_t tlv, uint32_t flags)
{
...
AppSpawnMsgNode *IsolateTestHelper::IsolateTestCreateAppSpawnMsg(AppSpawnMsg *msg)
{
...
AppSpawnReqMsgHandle IsolateTestHelper::IsolateTestCreateSendMsg(AppSpawnClientHandle handle, uint32_t msgType)
{
    AppSpawnReqMsgHandle reqHandle = 0;
    int ret = AppSpawnReqMsgCreate(static_cast<AppSpawnMsgType>(msgType), isolateTestProcessName_.c_str(), &reqHandle);
    APPSPAWN_CHECK(ret == 0, return INVALID_REQ_HANDLE, "Failed to create req %{public}s",
                   isolateTestProcessName_.c_str());
    APPSPAWN_CHECK_ONLY_EXPER(msgType == MSG_APP_SPAWN || msgType == MSG_SPAWN_NATIVE_PROCESS, return reqHandle);
    do {
        ret = IsolateTestAddFdInfo(reqHandle);
        APPSPAWN_CHECK(ret == 0, break, "Failed to add fd %{public}s", isolateTestProcessName_.c_str());
        ret = AppSpawnReqMsgSetBundleInfo(reqHandle, 100, isolateTestProcessName_.c_str());  // 100 test index
        APPSPAWN_CHECK(ret == 0, break, "Failed to add bundle info req %{public}s", isolateTestProcessName_.c_str());
        ret = IsolateTestAddDacInfo(reqHandle);
        APPSPAWN_CHECK(ret == 0, break, "Failed to add dac %{public}s", isolateTestProcessName_.c_str());
        ret = AppSpawnReqMsgSetAppAccessToken(reqHandle, 12345678);  // 12345678
        APPSPAWN_CHECK(ret == 0, break, "Failed to add access token %{public}s", isolateTestProcessName_.c_str());

        if (isolateTestDefaultMsgFlags_ != 0) {
            (void)IsolateTestSetMsgFlags(reqHandle, TLV_MSG_FLAGS, isolateTestDefaultMsgFlags_);
        }
        ...
        APPSPAWN_CHECK(ret == 0, break, "Failed to ext tlv %{public}s", isolateTestProcessName_.c_str());
        size_t count = isolateTestPermissions_.size();
        for (size_t i = 0; i < count; i++) {
            ret = AppSpawnReqMsgAddPermission(reqHandle, isolateTestPermissions_[i]);
            APPSPAWN_CHECK(ret == 0, break, "Failed to permission %{public}s", isolateTestPermissions_[i]);
        }

        ret = AppSpawnReqMsgSetAppInternetPermissionInfo(reqHandle, 1, 0);
        APPSPAWN_CHECK(ret == 0, break, "Failed to internet info %{public}s", isolateTestProcessName_.c_str());

        ret = AppSpawnReqMsgSetAppOwnerId(reqHandle, "ohos.permission.FILE_ACCESS_MANAGER");
        APPSPAWN_CHECK(ret == 0, break, "Failed to ownerid %{public}s", isolateTestProcessName_.c_str());
        ...
        APPSPAWN_CHECK(ret == 0, break, "Failed to render cmd %{public}s", isolateTestProcessName_.c_str());
        ret = AppSpawnReqMsgSetAppDomainInfo(reqHandle, 1, isolateTestDefaultApl_.c_str());
        APPSPAWN_CHECK(ret == 0, break, "Failed to domain info %{public}s", isolateTestProcessName_.c_str());
        return reqHandle;
    } while (0);
...
AppSpawningCtx *IsolateTestHelper::IsolateTestGetAppProperty(AppSpawnClientHandle handle,
    AppSpawnReqMsgHandle reqHandle)
{
    ...
    AppSpawnMsgNode *msgNode = IsolateTestCreateAppSpawnMsg(reqNode->msg);
