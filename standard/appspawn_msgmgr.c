...
static inline void DumpMsgFlags(const char *info, const AppSpawnMsgFlags *msgFlags)
{
    for (uint32_t i = 0; i < msgFlags->count; i++) {
        APPSPAWN_DUMP("%{public}s flags: 0x%{public}x", info, msgFlags->flags[i]);
    }
}
...
static inline void DumpMsgExtInfo(const AppSpawnTlv *tlv)
{
    if (tlv->tlvType != TLV_MAX) {
        APPSPAWN_DUMP("App tlv info: [%{public}d %{public}d]", tlv->tlvType, tlv->tlvLen);
        return;
    }
    AppSpawnTlvExt *tlvExt = (AppSpawnTlvExt *)(tlv);
    if (tlvExt->dataType == DATA_TYPE_STRING) {
        APPSPAWN_DUMP("App extend info name: %{public}s len: %{public}u value: '%{public}s'",
...
void DumpAppSpawnMsg(const AppSpawnMsgNode *message)
{
    ...
    APPSPAWN_ONLY_EXPER(owner != NULL, APPSPAWN_DUMP("App owner info: \"%{public}s\" ", owner->ownerId));
    ...
    APPSPAWN_ONLY_EXPER(info != NULL,
        APPSPAWN_DUMP("App internet permission info [%{public}d %{public}d]",
